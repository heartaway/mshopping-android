...
package com.taobao.tae.Mshopping.demo.view.pinterest;
 ...
 * the {@link android.widget.ListAdapter} associated with this view.
 *
 * <p>
 ...
 */
public class PinterestListView extends PinterestAbsListView {
    ...
    public boolean removeHeaderView(View v) {
        if (mHeaderViewInfos.size() > 0) {
            ...
            if (((PinterestHeaderViewListAdapter) mAdapter).removeHeader(v)) {
    ...
    public boolean removeFooterView(View v) {
        if (mFooterViewInfos.size() > 0) {
            boolean result = false;
            if (((PinterestHeaderViewListAdapter) mAdapter).removeFooter(v)) {
                mDataSetObserver.onChanged();
                result = true;
            }
     ...
     * {@link #setAdapter(android.widget.ListAdapter)} but might be a
     * {@link android.widget.WrapperListAdapter}.
     *
     ...
     * {@link android.widget.WrapperListAdapter}, depending on the ListView features currently
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
            mAdapter = new PinterestHeaderViewListAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
        } else {
            mAdapter = adapter;
        }
    ...
    final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition, final int maxHeight,
        ...
        final PinterestAbsListView.RecycleBin recycleBin = mRecycler;
    ...
    private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft, boolean selected, boolean recycled) {
        ...
        PinterestAbsListView.LayoutParams p = (PinterestAbsListView.LayoutParams) child.getLayoutParams();
        if (p == null) {
            p = new PinterestAbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0);
        }
        p.viewType = mAdapter.getItemViewType(position);
    ...
    private void scrollListItemsBy(int amount) {
        ...
        if (amount < 0) {
            ...
            while (first.getBottom() < listTop) {
                PinterestAbsListView.LayoutParams layoutParams = (LayoutParams) first.getLayoutParams();
            ...
            while (last.getTop() > listBottom) {
                PinterestAbsListView.LayoutParams layoutParams = (LayoutParams) last.getLayoutParams();
                if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
